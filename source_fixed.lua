-- TurtleSpy V1.5.3 (Fixed), credits to Intrer#0421
-- Bug fixes by Claude:
-- 1. OldEvent & OldFunction sekarang memanggil fungsi aslinya (FIX UTAMA - penyebab crash)
-- 2. addToList dibungkus pcall di dalam semua hook
-- 3. getcallingscript() dibungkus pcall agar aman di exploit selain Synapse
-- 4. convertTableToString dilindungi dari circular table (stack overflow)
-- 5. mouse.KeyDown diganti UserInputService (deprecated fix)
-- 6. FindRemote dibungkus pcall
...
local function GetFullPathOfAnInstance(instance)
    local ok, result = pcall(function()
        ...
        elseif instance == workspace then
            ...
            local _success, svcResult = pcall(game.GetService, game, instance.ClassName)
    ...
    if ok then return result else return tostring(instance) end
end
...
-- [FIX MOBILE] Tombol toggle layar untuk membuka/tutup GUI (support mobile & PC)
local ToggleButton = Instance.new("ImageButton")
local ToggleButtonGui = Instance.new("ScreenGui")
ToggleButtonGui.Name = "TurtleSpyToggle"
...
ToggleButton.BackgroundColor3 = Color3.fromRGB(0, 168, 255)
ToggleButton.BorderSizePixel = 0
ToggleButton.ZIndex = 999
ToggleButton.Active = true
ToggleButton.Draggable = true
ToggleButton.Image = "rbxassetid://169476802"
...
-- Tetap support keybind keyboard untuk pengguna PC
local UserInputService = game:GetService("UserInputService")
...
local function FindRemote(remote, args)
    -- [FIX] Bungkus dengan pcall agar tidak crash jika ada error
    local ok, result = pcall(function()
...
-- [FIX] convertTableToString sekarang tracking visited tables agar tidak stack overflow (circular reference)
local function convertTableToString(args, _visited)
    ...
    for i,v in pairs(args) do
        ...
        elseif type(v) == "table" then
            -- [FIX] Cek circular reference agar tidak infinite loop / stack overflow
            if _visited[v] then
                string = string .. "{--[[circular]]}"
            else
                _visited[v] = true
                string = string .. "{"
                string = string .. convertTableToString(v, _visited)
                string = string .. "}"
                _visited[v] = nil
            end
...
function addToList(event, remote, ...)
    -- [FIX] Bungkus seluruh fungsi dengan pcall agar error tidak crash exploit
    local ok, err = pcall(function()
        ...
        if not i then
            ...
            -- [FIX] getcallingscript() dibungkus pcall agar aman di non-Synapse exploit
            local callingScript = nil
            if isSynapse() then
                local s, r = pcall(getcallingscript)
                if s then callingScript = r end
            else
                callingScript = rawget(getfenv(0), "script")
            end
            remoteScripts[#remotes] = callingScript
    ...
    if not ok then
        warn("[TurtleSpy] addToList error: " .. tostring(err))
    end
end

-- ============================================================
-- [FIX UTAMA] OldEvent sekarang memanggil fungsi asli di akhir
-- Sebelumnya: fungsi asli TIDAK pernah dipanggil → game rusak → crash
-- ============================================================
local OldEvent
OldEvent = hookfunction(Instance.new("RemoteEvent").FireServer, newcclosure(function(Self, ...)
    if not checkcaller() and table.find(BlockList, Self) then
        return
    elseif table.find(IgnoreList, Self) then
        return OldEvent(Self, ...)
    end
    -- [FIX] Bungkus addToList dengan pcall agar error tidak crash hook
    pcall(addToList, true, Self, ...)
    return OldEvent(Self, ...) -- [FIX] Wajib ada! Panggil fungsi aslinya
end))

-- ============================================================
-- [FIX UTAMA] OldFunction sekarang memanggil fungsi asli di akhir
-- ============================================================
local OldFunction
OldFunction = hookfunction(Instance.new("RemoteFunction").InvokeServer, newcclosure(function(Self, ...)
    if not checkcaller() and table.find(BlockList, Self) then
        return
    elseif table.find(IgnoreList, Self) then
    ...
    -- [FIX] Bungkus addToList dengan pcall agar error tidak crash hook
    pcall(addToList, false, Self, ...)
    return OldFunction(Self, ...) -- [FIX] Wajib ada! Panggil fungsi aslinya
end))
...
OldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    if method == "FireServer" and isA(Self, "RemoteEvent") then
        ...
        -- [FIX] Bungkus addToList dengan pcall
        pcall(addToList, true, ...)
